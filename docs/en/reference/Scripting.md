...
| Function | Description 
| -------- | ----------- |
| `saveMedia(folder: string, generateFileNames: boolean): [{ name: string, size: long, folder: string, mediaPath: string, hasError: boolean, errorMessage: string }]` | Saves all files present on the HttpRequest to a specific folder. Filenames can be generated by passing true to the second argument |

The `saveMedia` returns an array of `SaveMediaResult` objects. Each object in the array represents a file that was uploaded. If an error occurs when trying to save a file, the `hasError` boolean property will be true and the `errorMessage` property will be populated with the error message.
